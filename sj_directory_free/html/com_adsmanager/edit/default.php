...
?>
<div class="juloawrapper" id="<?php echo $this->content->id ?>">
<?php
...
if ($nbimages > 0) {
	...
	JText::script('ADSMANAGER_VALIDATE_POSITIVE_NUMBER');
	...
	$document->addScriptDeclaration("
		jQ.extend(jQ.validator.messages, {
			...
			price				: ".json_encode(JText::_('ADSMANAGER_VALIDATE_POSITIVE_NUMBER')).",
...
<form action="<?php echo $target;?>" method="post" class="form-horizontal" name="adminForm" id="adminForm" enctype="multipart/form-data" onkeypress="return checkEnter(event)" onsubmit="return submitbutton(this)">
  ...
  <div id="formcontainer">
   <!-- category -->
		...
		$visibility_pattern = "";
		if($this->catid == 10 || $this->catid == 23 || $this->catid == 86 || $this->catid == 87){
			$visibility_pattern = 'style="display: none"';
		}else{
			if($this->isUpdateMode) {
			   echo JText::_('ADSMANAGER_STEP_SELECT_CATEGORY');
			 }
			 else {
			   echo JText::_('ADSMANAGER_STEP_SELECT_CATEGORY')." ".JText::_('ADSMANAGER_REQUIRED');
			 }
		}
        ...
        <?php
          ...
          if ($this->nbcats == 1)
          {
            ...
            <div class="control-group" >
                ...
                <?php endif; ?>
                <div class="controls sub_cats" <?php echo $visibility_pattern; ?> >
                <?php
		...
		jQ(document).ready(function() {
			jQ('.price_container_after input').focusout(function() {
				var elem = jQ('#fad_prixloyer').val();
				if(elem < 0){
					var theprice = elem.replace('-','');
					jQ('#fad_prixloyer').val(theprice);
				}
			});
			
			jQ('.price_container_after input').focusout(function() {
				var elem = jQ('#fad_price').val();
				if(elem < 0){
					var theprice = elem.replace('-','');
					jQ('#fad_price').val(theprice);
				}
			});
	   ...
	   )
	{
                    ...
                    if(!isset($field->options->display_edit_title) || $field->options->display_edit_title == 1){
						...
						if ($field->required == 1)
						 ...
						 if ((@$field->description)&&($field->description !="")) {
							...
							if(($field->name=="ad_headline") && ($this->rootid == 21 || $this->rootid == 25)){
								echo "<label class=\"control-label\" for=\"{$field->name}\">".JText::_('ADSMANAGER_JOB_POSITION').$requiredtxt."</label>";
							}else if(($field->name=="ad_headline") && ($this->rootid != 21 || $this->rootid != 25)){
								echo "<label class=\"control-label\" for=\"{$field->name}\">".JText::_('ADSMANAGER_FORM_MESSAGE_TITLE').$requiredtxt."</label>";
							}else if(($field->name=="ad_text") && ($this->rootid == 21 || $this->rootid == 25)){
								echo "<label class=\"control-label\" for=\"{$field->name}\">".JText::_('ADSMANAGER_JOB_DESCRIPTION').$requiredtxt."</label>";
							}else
		...
		<?php
			...
			// $cats1 = $catmodel->getFlatTree(true, false, $nbContents, 'write',$rootid);
			
			if ($withImages) {
				$pic = ($this->rootid == 21 || $this->rootid == 25) ? JText::_('ADSMANAGER_FORM_AD_COMPANYLOGO') : JText::_('ADSMANAGER_FORM_AD_PICTURE');
				?>
				<h3>
					<?php echo $pic; ?>
				</h3>
                    ...
                    <div class="row-fluid">
                        <div class="span12">
			                <div class="control-group">
			                    ...
			                    <?php echo TImage::displayImageUploader($this->conf,$this->content,$this->adext,$this->rootid);?>
			                    <?php if (PAIDSYSTEM) {
...
function checkdependency(child,parentname,parentvalues,cleanValue) {
	...
	jQ(document).ready(function () { 
		jQ('#row_ad_customcategory .control-group').hide(); 
		
		jQ('#fad_hobbietype').on('change', function (e) {
			var optionSelected = jQ("option:selected", this);
			var valueSelected = this.value;
			
			if(valueSelected == "Others"){
				jQ('#row_ad_customcategory .control-group').show();
			}else{
				jQ('#row_ad_customcategory .control-group').hide();
				jQ('#row_ad_customcategory input').val('');
			}
		});
		
		jQ('#row_ad_categorieutilisateur .control-group').hide(); 
		
		jQ('#fad_typeloisir').on('change', function (e) {
			var optionSelected = jQ("option:selected", this);
			var valueSelected = this.value;
			
			if(valueSelected == "Autres"){
				jQ('#row_ad_categorieutilisateur .control-group').show();
			}else{
				jQ('#row_ad_categorieutilisateur .control-group').hide();
				jQ('#row_ad_categorieutilisateur input').val('');
			}
		});
	});
