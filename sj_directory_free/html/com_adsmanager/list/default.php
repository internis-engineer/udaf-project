...
<div class="row-fluid">	
	<?php
		if($this->pagination->total == 0){ 
			...
			$nbadtext = ($this->pagination->total == 1) ? JText::_('ADSMANAGER_AD_IN') : JText::_('ADSMANAGER_ADS_IN');
			// var_dump($this);
			?>
			<h1 class="adheadingtitle"><span><?php echo $this->pagination->total."</span> ".$nbadtext." ".JText::_($this->list_name) ?></h1>
			<?php
		}
	?>
</div>
...
<?php foreach($this->subcats as $key => $subcat) {
	...
	if($subcat->id == "11" || $subcat->id !="18" || $subcat->id !="19"){
		echo "";
	}else{
		if ($key != 0)
		echo ' | ';
