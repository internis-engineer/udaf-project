...
<div class="juloawrapper">
    ...
    <?php if ($search_by_text == 1){ ?>
                <div class="<?php echo $class; ?>">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input class="search-field" type="text" name="tsearch" placeholder="<?php echo JText::_('ADSMANAGER_LIST_SEARCH'); ?>" value="<?php echo $text_search; ?>" />
                        </div>
                    </div>
                </div>
    <?php }?>
        ...
        <?php 
			...
			if(($idcat) && $idcat != 0){
				echo '<input type="hidden" name="catid" id="catid-'.$moduleId.'" value="'.$idcat.'" />';
			}else{
				if($rootid == 17){
					echo '<input type="hidden" name="catid" id="catid-'.$moduleId.'" value="'.$rootid.'" />';
				}else{
    ...
    jQ(document).ready(function() {
		...
		// jQ('#modsimplesearch-form .btn-primary').click(function(){
			// var inputfield = jQ('#modsimplesearch-form input').val();
			
			
			// jQ('#modsimplesearch-form').submit(function() {
				// jQ('#modsimplesearch-form .error').html('<p>Submit blocked!</p>');
				// return false;
			// });
		// });
        ...
        <?php
            if (isset($searchfields)) {
                foreach($searchfields as $fsearch) { 
					...
					searchmandatory('<?php echo $fsearch->name?>');
                    <?php 
                    } 
                }
            }
